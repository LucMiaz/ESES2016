...
def match(*args,**kwargs):
    """
    will return a list of matching elements. required is the label of the desired element (either for node or relationship, e.g. "Site" or "OF"). 
    to get a relationship, please give also at least the origin node as "origin" and the target node as "target". You can also give other
    """
    try:
        ...
        properties=kwargs
        try:
            relation=properties.pop('relationshipNode')
        except:
            relation=False
            
        if relation==True:
            if "target" in properties.keys() and "origin" in properties.keys():
                targetNode=properties.pop('target')
                originNode=properties.pop('origin')
                if isinstance(targetNode, Node) and isinstance(originNode,Node):
                    command='MATCH (k:'+originNode.element_type+originNode.__props__+')-[n:'+label.upper()+']->(m:'+targetNode.element_type+targetNode.__props__+')'
                    if len(properties.keys())>0:
                        command+= command+" WHERE n="+dictToStr(properties)
                    command+=command+" RETURN n"
                    result=RUN(command)
                    ...
                    for record in result_list:
                        ...
                        klass=Relationship(originNode, label, targetNode, createN=False, **props)
                        returnlist.append(klass)
                    return(returnlist)
                else:
                    result=None
                    print("target and origin are not of class Node")
                    return(None)
            else:
                result=None
                print("No target node or/and origin node found")
        else:
...
class Relationship(object):
    ...
    def __init__(self, Node1, label, Node2,createN=True,**kwargs):
        self.n1=Node1
        self.n2=Node2
        self.label=label
        self.props=kwargs
